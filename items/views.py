...
def additem(request):
    if request.method == 'POST':
        ...
        item = Item(ownermail=omail,start_date=sdate,currentPrice=price,img1=img1,img2=img2,name=iname,profile=prof,tag=itag,short_description=sdisc,long_description=ldisc,basePrice=price)
        item.save()
        send_new_item_email(item)
        return redirect("home")
    else:
        return render(request,'additem.html')
@login_required(login_url='login')
@user_passes_test(lambda u: u.is_superuser)
def verify_item(request, item_iname):
    item = Item.objects.get(id=item_iname)
    item.verified = True
    item.save()
    return redirect('home')
...
def validate(request):
    ...
    if bidderEmail==itemownerEmail:
        ...
        msg     = "Congratulations your item is bidded by "+bidder.email+", By NPR rs = "+value+". Contact your buyer by email Thank You for using our app."
        to      = mail  
...
def send_new_item_email(item):
    email_content = "New {} has been added for bidding!! Hurry up to get the best deal ".format(item.name)
    subject = 'New item added: {}'.format(item.name)
    to = [user.email for user in User.objects.all()]
    email_message = EmailMessage(subject=subject, body=email_content, to=to)
